...
export const selectListCart: MemoizedSelector<AppState, any | null, DefaultProjectorFn<any | null>> = createSelector(
  selectExerciseFeature,
  (state: CartState) => state[CartFeatureKey]
);

export const selectTotal: MemoizedSelector<AppState, any | null, DefaultProjectorFn<any | null>> = createSelector(
  selectExerciseFeature,
  (state: CartState) => {
    ...
    return values.reduce((prev, current) => {
      return Number(prev) + Number(current?.quantity) * Number(current?.article?.priceSell || 0);
    }, 0);
  }
);
